...
class Game
{
    ...
    const ATTACHED_HU_SANJIAQING = 64; //三家清
    ...
    public function judge_hu($chair, $is_fanhun = false)
    {
        ...
        if($this->m_bHaveGang && $this->m_sGangPao->mark && $this->m_sGangPao->chair == $chair)	//杠开
        {
    ...
    public function judge_hu_type($chair)
    {
        ...
        if ($bType32)
        {
            ...
            if (($huCard_type != ConstConfig::PAI_TYPE_FENG && $huCard_key < 8 && $this->m_sPlayer[$chair]->card[$huCard_type][$huCard_key + 1] > 0 && $this->m_sPlayer[$chair]->card[$huCard_type][$huCard_key + 2] > 0)) {
                if ($this->judge_32type($chair, array($this->m_HuCurt[$chair]->card, $this->m_HuCurt[$chair]->card + 1, $this->m_HuCurt[$chair]->card + 2)))
                {
                    $bBian = true;
                }
            ...
            {
                if ($this->judge_32type($chair, array($this->m_HuCurt[$chair]->card, $this->m_HuCurt[$chair]->card, $this->m_HuCurt[$chair]->card)))
                {
                    $bDuidao = true;
                }
            ...
            {
                if ($this->judge_3type($chair, array($this->m_HuCurt[$chair]->card, $this->m_HuCurt[$chair]->card)))
                {
                    //还要判断一下听的牌是不是只有将牌
                    if ($this->judge_dantingjiang($chair))
                    {
                        $bDandiao = true;
                    }
                }
            ...
            //手把一
            if ($this->m_sPlayer[$chair]->len == 2)//手把一判断
            {
                $bShoubayi = true;
            }
            //胡19
            ...
            for ($i = 0; $i < $this->m_rule->player_count; $i++)
            {
                for ($j = 0; $j < $this->m_sStandCard[$i]->num; $j++)
                {
                    if (ConstConfig::DAO_PAI_TYPE_KE == $this->m_sStandCard[$i]->type[$j])
                    {
                        ...
                        {
                            /*var_dump(__LINE__);
                            var_dump($this->judge_danting($chair,$this->m_HuCurt[$chair]->card));*/
            ...
            //如果牌型为1,2,3,3,3胡3或者7,7,7,8,9胡7，这个要显示特胡
            /*if(in_array($this->m_HuCurt[$chair]->card,array(3,19,35)))
            {
                if ($bHupai4zhang)
                {
            ...
            //抢杠胡显示特胡
            /*var_dump(__line__);
            var_dump($this->m_sQiangGang->mark);
            var_dump($this->m_sQiangGang->card);*/
            if($this->m_sQiangGang->mark)
            {
    ...
    //判断删除牌后是否满足3牌型
    public function judge_3type($chair,$delCard_arr)
    {
        ...
        $result=$this->judge_3Cardtype($chair);
    ...
    //判断是否符合3牌型
    public function judge_3Cardtype($chair)
    {
        $jiang_arr = array();
    ...
    //判断单听将牌
    public function judge_dantingjiang($chair)
    {
        $jiang_arr = array();
    ...
    public function judge_jiang($chair,$insertCard)
    {
        ...
        {
            if ($this->judge_3type($chair,array($insertCard,$insertCard)))
            {
                {
                    $bJiang = true;
                }
    ...
    public function judge_she_te($chair,$huCard)
    {
        ...
        if ($bType32)
        {
            ...
            //如果牌型为1,2胡3或者8,9胡7，这个要显示夹，2番
            if(in_array($huCard,array(3,19,35)))
            {
                if($this->m_sPlayer[$chair]->card[$huCard_type][1]>0 && $this->m_sPlayer[$chair]->card[$huCard_type][2]>0)
                {
                    $bJiahu = true;
            ...
            if(in_array($huCard,array(7,23,39)))
            {
                if($this->m_sPlayer[$chair]->card[$huCard_type][8]>0 && $this->m_sPlayer[$chair]->card[$huCard_type][9]>0)
                {
                    $bJiahu = true;
	...
	{
        //海底区(最后牌堆剩下<=6张牌的时候，为海底区，在海底区时，不能开杠。)
        if ($this->m_nCountAllot >= $this->m_nAllCardNum-5)
        {
            return false;
        }
		$temp_card = $gang_card;
	...
	public function HandleChooseResult($chair, $nCmdID, $eat_num = null)
	{
		...
		if ($this->m_sQiangGang->mark )	// 处理抢杠
		{
			...
			else // 给杠的玩家补张
			{
                ...
                $this->m_only_out_card[$this->m_sQiangGang->chair] = false;
	...
	public function OnGetChairScene($chair, $is_more=false)
	{
        ...
        /*$data['m_she_te'] = $this->m_she_te;	//设特*/
        ...
        else if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_CHI_PAO)
        {
            ...
            if(!(in_array(self::ATTACHED_HU_SANJIAQING, $this->m_HuCurt[$chair]->method)))
            {
                if (!empty($this->m_rule->is_qixiaodui))
                {
                    ...
                    {
                        if (!$this->judge_ting_qidui($lost_chair))
                        {
                            $banker_fan = $banker_fan * 2;
