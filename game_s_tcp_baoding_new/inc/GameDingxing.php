...
use gf\inc\Game_cmd;
class GameDingxing extends BaseGame
{
    const GAME_TYPE = 124;
    //－－－－－－－－－－－－－胡牌类型 －－－－－－－－－－－－－－－－－－－
    ...
    public static $hu_type_arr = array(
        ...
        self::HU_TYPE_QIDUI=>[self::HU_TYPE_QIDUI, 5, '七对']
    );

    public static $attached_hu_arr = array(
        self::ATTCHED_HU_ZHUOWUKUI=>[self::ATTCHED_HU_ZHUOWUKUI, 3, '捉五魁'],
        self::ATTCHED_HU_KOUPAI=>[self::ATTCHED_HU_KOUPAI, 2, '扣牌'],
        self::ATTCHED_KOUDAJIANG=>[self::ATTCHED_KOUDAJIANG, 4, '扣大将'],
        self::ATTACHED_HU_YITIAOLONG=>[self::ATTACHED_HU_YITIAOLONG, 5, '一条龙'],
    ...
    public function _open_room_sub($params)
    {
        ...
        $params['rule']['is_chipai'] = !isset($params['rule']['is_chipai']) ? 1 : $params['rule']['is_chipai'];
	...
	public function judge_hu($chair, $is_fanhun = false)
	{
		...
		$is_yitiaolong = false;
		$is_wukui = false;
		$hu_type = $this->judge_hu_type($chair, $is_yitiaolong, $is_wukui);
	...
	public function judge_fan($chair)
	{
		...
		if(isset(self::$hu_type_arr[$hu_type]))
		{
			...
			$tmp_hu_desc_base = self::$hu_type_arr[$hu_type][2].' ';
		}

		$attached_fan_sum = 0;
        $tmp_hu_desc_attached = '';
		for($i=1; $i<$this->m_HuCurt[$chair]->count; $i++)
		{
			if(isset(self::$attached_hu_arr[$this->m_HuCurt[$chair]->method[$i]]))
			{
				...
				$tmp_hu_desc_attached .= self::$attached_hu_arr[$this->m_HuCurt[$chair]->method[$i]][2].' ';
			}
		}
		...
		{
            if($this->m_sPlayer[$chair]->kou_card[3][1] == 1 || $this->m_sPlayer[$chair]->kou_card[3][1] == 3)
			{
                $is_kou = true;
                $kou_fan_sum = 2;
				$tmp_hu_desc_kou = '扣牌 ';
			...
			if(!empty($this->m_rule->is_kou_dajiang) && $is_kou)
			{
                $jiangCount = 0;
				$kou_card_array = array();
				for ($j=0; $j < 4; $j++)
				{
                    $kou_card_type = $this->_get_card_type($this->m_sPlayer[$chair]->kou_card[$j][0]);
                    if ($kou_card_type == ConstConfig::PAI_TYPE_FENG)
                    {
                        $kou_card_array[] = $this->m_sPlayer[$chair]->kou_card[$j][0];
                    }
				}
                $count = array_count_values($kou_card_array);
        ...
        //保定附加番的时候不计平胡
        if($hu_type == self::HU_TYPE_PINGHU && (!empty($attached_fan_sum) || !empty($kou_fan_sum)))
        {
            $fan_sum = $attached_fan_sum + $kou_fan_sum;
            $tmp_hu_desc .= ($tmp_hu_desc_attached.$tmp_hu_desc_kou);
        }
        else
        {
            $fan_sum = $fan_sum + $attached_fan_sum + $kou_fan_sum;
            $tmp_hu_desc .= ($tmp_hu_desc_base.$tmp_hu_desc_attached.$tmp_hu_desc_kou);
        }
	...
	//胡牌类型判断  没有混的情况
	public function judge_hu_type($chair, &$is_yitiaolong, &$is_wukui)
	{
		$jiang_arr = array();
		$qidui_arr = array();
	...
	public function HandleChooseAnGang($chair, $gang_card)
	{
        ...
        $this->m_sQiangGang->init_data(true, $temp_card, $chair, 1);
        $this->m_sOutedCard->clear();

        //若有人可以胡，抢杠胡,重置抢胡牌命令个数
        $this->m_nNumCmdHu = 0;
	...
	public function HandleChooseResult($chair, $nCmdID, $eat_num = null)
	{
		...
		if ($this->m_sQiangGang->mark)
		{
            ...
            if ($this->m_sQiangGang->type == 1)
            {
                $this->_list_insert($chair, $temp_card);
                $bhu = $this->judge_hu($chair);
                $this->_list_delete($chair, $temp_card);

                if ($this->m_nNumCmdHu && $bhu && in_array(self::ATTACHED_HU_YITIAOLONG,$this->m_HuCurt[$chair]->method))
                {
                    ...
                    //还原暗杠手牌
                    $this->_list_insert($this->m_sQiangGang->chair, $this->m_sQiangGang->card);
                    $this->_list_insert($this->m_sQiangGang->chair, $this->m_sQiangGang->card);
                    $this->_list_insert($this->m_sQiangGang->chair, $this->m_sQiangGang->card);

                    // 设置倒牌
                    array_pop($this->m_sStandCard[$this->m_sQiangGang->chair]->type);
                    array_pop($this->m_sStandCard[$this->m_sQiangGang->chair]->first_card);
                    array_pop($this->m_sStandCard[$this->m_sQiangGang->chair]->card);
                    array_pop($this->m_sStandCard[$this->m_sQiangGang->chair]->who_give_me);
                    $this->m_sStandCard[$this->m_sQiangGang->chair]->num = $this->m_sStandCard[$this->m_sQiangGang->chair]->num -1;

                    //还原扣牌
                    $this->_recover_koucard($this->m_sQiangGang->chair, $this->m_sQiangGang->card);
                ...
                else
                {
                    ...
                    $this->m_sGangPao->init_data(true, $this->m_sQiangGang->card, $this->m_sQiangGang->chair, ConstConfig::DAO_PAI_TYPE_ANGANG, $nGangPao);

                    $this->_set_record_game(ConstConfig::RECORD_ANGANG, $this->m_sQiangGang->chair, $this->m_sQiangGang->card, $this->m_sQiangGang->chair);

                    $this->m_wTotalScore[$this->m_sQiangGang->chair]->n_angang += 1;
                    $this->countGang += 1;
                    //更改状态
                    $this->m_sysPhase = ConstConfig::SYSTEMPHASE_THINKING_OUT_CARD;
	...
	public function ScoreOneHuCal($chair, &$lost_chair)  
	{
		...
		if ($this->isUpperChairDaBao[$chair] == 1)
        {
            $tmp_win_score = 0;
            $upper_chair = $this->_anti_clock($chair,$this->m_rule->player_count -1);
            for($i = 0; $i < $this->m_rule->player_count; $i++)
            {
                ...
                $wWinScore = 0;
                $wWinScore +=  ConstConfig::SCORE_BASE * $PerWinScore * $dianpao;

                $tmp_win_score += $wWinScore;

                $this->m_HuCurt[$chair]->gain_chair[0]++;
                $this->m_HuCurt[$chair]->gain_chair[$this->m_HuCurt[$chair]->gain_chair[0]] = $upper_chair;
            }

            $tmp_win_score = $this->_get_max_fan($tmp_win_score);

            $this->m_wHuScore[$upper_chair] -= $tmp_win_score;
            $this->m_wHuScore[$chair] += $tmp_win_score;

            $this->m_wSetLoseScore[$upper_chair] -= $tmp_win_score;
            $this->m_wSetScore[$chair] += $tmp_win_score;

            return true;
        }
		else if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)
		{
			for($i = 0; $i < $this->m_rule->player_count; $i++)
			{
				if($i == $chair || $this->m_sPlayer[$i]->state == ConstConfig::PLAYER_STATUS_BLOOD_HU)
				...
				$PerWinScore = ($PerWinScore == 0)? 1 : $PerWinScore;
				$wWinScore = 0;
				$wWinScore += 2 * ConstConfig::SCORE_BASE * $PerWinScore;  //自摸翻倍了
		...
		else if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_CHI_PAO)
		{
			...
			else if(empty($this->m_rule->is_dianpao_bao))
			{
				$PerWinScore = ($PerWinScore == 0)? 1 : $PerWinScore;	
				$wWinScore = 0;
				$wWinScore += 2 * ConstConfig::SCORE_BASE * $PerWinScore;	//点炮加倍
    ...
    public function _is_yitiaolong($chair, &$is_yitiaolong)
    {
        ...
        for($i=0; $i<$this->m_sStandCard[$chair]->num; $i++)
        {
            if ($this->m_sStandCard[$chair]->type[$i] == ConstConfig::DAO_PAI_TYPE_SHUN)
            {
        ...
        if ($is_yitiaolong)
        {
            for($i=0; $i<$this->m_sStandCard[$chair]->num; $i++)
            {
                if ($this->m_sStandCard[$chair]->type[$i] == ConstConfig::DAO_PAI_TYPE_SHUN)
                {
                	$card_type = $this->_get_card_type($this->m_sStandCard[$chair]->first_card[$i]);
	                $card_index = $this->m_sStandCard[$chair]->first_card[$i] % 16;
	                if (isset($cardTypeOfLong) && $card_type == $cardTypeOfLong)
	                {
	                    $standCard[] = $card_index;
	                }
                }  
            }

            $hucard_type = $this->_get_card_type($this->m_HuCurt[$chair]->card);
            $hucard_index = $this->m_HuCurt[$chair]->card % 16;

            if (!empty($standCard))
            {
                if (in_array(1, $standCard) && in_array(4, $standCard) && in_array(7, $standCard))
                {
                    $this->isUpperChairDaBao[$chair] = 1;
                }
                elseif(!in_array(1, $standCard) && in_array(4, $standCard) && in_array(7, $standCard))
                {
                	if(isset($cardTypeOfLong) && $hucard_type == $cardTypeOfLong && ($hucard_index == 1 || $hucard_index == 2 || $hucard_index == 3))
                	{
                		$used_in_long = $this->_used_in_long($chair, $hucard_type, $hucard_index);
                		if($used_in_long && $this->m_nChairDianPao == $this->_anti_clock($chair, -1))
                		{
                			$this->isUpperChairDaBao[$chair] = 1;
                		}
                	}
                ...
                elseif(in_array(1, $standCard) && !in_array(4, $standCard) && in_array(7, $standCard))
                {
                	if(isset($cardTypeOfLong) && $hucard_type == $cardTypeOfLong && ($hucard_index == 4 || $hucard_index == 5 || $hucard_index == 6))
                	{
                		$used_in_long = $this->_used_in_long($chair, $hucard_type, $hucard_index);
                		if($used_in_long && $this->m_nChairDianPao == $this->_anti_clock($chair, -1))
                		{
                ...
                elseif(in_array(1, $standCard) && in_array(4, $standCard) && !in_array(7, $standCard))
                {
                	if(isset($cardTypeOfLong) && $hucard_type == $cardTypeOfLong && ($hucard_index == 7 || $hucard_index == 8 || $hucard_index == 9))
                	{
                		$used_in_long = $this->_used_in_long($chair, $hucard_type, $hucard_index);
                		if($used_in_long && $this->m_nChairDianPao == $this->_anti_clock($chair, -1))
                		{
    ...
    public function _used_in_long($chair, $hucard_type, $hucard_index)
    {
    	$return = false;
    	$chi_index = $hucard_index % 3;
    	if($chi_index == 1)
    	{
    		$tmp_index_first = $hucard_index;
    		$tmp_index_second = $hucard_index +1;
    		$tmp_index_third = $hucard_index +2;
    	}
    	if($chi_index == 2)
    	{
    		$tmp_index_first = $hucard_index -1;
    		$tmp_index_second = $hucard_index;
    		$tmp_index_third = $hucard_index +1;
    	}
    	if($chi_index == 0)
    	{
    		$tmp_index_first = $hucard_index -2;
    		$tmp_index_second = $hucard_index -1;
    		$tmp_index_third = $hucard_index;
    	}
    	if($this->m_sPlayer[$chair]->card[$hucard_type][$tmp_index_first] == 0 || $this->m_sPlayer[$chair]->card[$hucard_type][$tmp_index_second] == 0 || $this->m_sPlayer[$chair]->card[$hucard_type][$tmp_index_third] == 0)
    	{
    		return $return;
    	}
    	$tmp_first = $this->_get_card_index($hucard_type, $tmp_index_first);
    	$tmp_second = $this->_get_card_index($hucard_type, $tmp_index_second);
    	$tmp_third = $this->_get_card_index($hucard_type, $tmp_index_third);
    	
    	$this->_list_delete($chair, $tmp_first);
    	$this->_list_delete($chair, $tmp_second);
    	$this->_list_delete($chair, $tmp_third);

    	$tmp_hu_data = &ConstConfig::$hu_data;
    	$key = intval(implode('', array_slice($this->m_sPlayer[$chair]->card[$hucard_type], 1)));
        if(isset($tmp_hu_data[$key]) && ($tmp_hu_data[$key] & 1) == 1)
        {
            $return = true;
        }

        $this->_list_insert($chair, $tmp_first);
    	$this->_list_insert($chair, $tmp_second);
    	$this->_list_insert($chair, $tmp_third);

    	return $return;
    }

    public function _is_wukui($chair, &$is_wukui, $bQiDui)
	{
		$is_wukui = false;
		$bType32 = false;
    ...
    public function _recover_koucard($chair, $gangCard)
    {
        if(!empty($this->m_rule->is_kou_card))
        {
            $tmp_times = 0;
            for ($i=0; $i < 4; $i++)
            {
                if($this->m_sPlayer[$chair]->kou_card[$i][0] == $gangCard && $this->m_sPlayer[$chair]->kou_card[$i][1] == 3)
                {
                    $this->m_sPlayer[$chair]->kou_card[$i][1] = 1;
                    $tmp_times++;
                }
