...
class GameWenAn extends BaseGame
{
    const GAME_TYPE = 421;
	//－－－－－－－－－－－－－胡牌类型 －－－－－－－－－－－－－－－－－－－
			...
			if(!empty($params['uid']))
			{
				foreach ($this->m_room_players as $key => $room_user)
				{
					if($room_user['uid'] == $params['uid'])
					{
						...
						$cmd = new Game_cmd($this->m_room_id, 's_get_game', $this->OnGetChairScene_test($key, true), Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$key]['uid']);
						$cmd->send($this->serv);
						unset($cmd);
    ...
    //掉线玩家重新回到游戏，取数据
    public function c_get_act($fd, $params)
    {
        $return_send = array("act"=>"s_result","info"=>__FUNCTION__ , "code"=>0, "desc"=>__LINE__.__CLASS__);
        do {
            ...
            if(!empty($params['uid']))
            {
                foreach ($this->m_room_players as $key => $room_user)
                {
                    if($room_user['uid'] == $params['uid'])
                    {
                        ...
                        {
                            $data['m_sDraw']['chair'] = $this->m_chairCurrentPlayer;
                            if ($this->m_chairCurrentPlayer == $key)
                            {
                                $data['m_sDraw']['card'] =  $this->m_sPlayer[$key]->card_taken_now;
                                $this->_send_cmd('s_draw',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$key]['uid']);
                                unset($data['m_sDraw']['card']);
                            }
                            else
                            {
                                $this->_send_cmd('s_draw',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$key]['uid']);
                        ...
                        if ($this->m_sysPhase == ConstConfig::SYSTEMPHASE_CHOOSING)
                        {
                            ...
                            $data['m_sOutedCard']['chair'] = $this->m_sOutedCard->chair;
                            $data['m_sOutedCard']['card'] = $this->m_sOutedCard->card;
                            $this->_send_cmd('s_discard',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$key]['uid']);
                        ...
                        if ($this->m_sysPhase == ConstConfig::SYSTEMPHASE_LA_ZHUANG)
                        {
                            ...
                            //发消息
                            $this->_send_cmd('s_lazhuang', $data, Game_cmd::SCO_SINGLE_PLAYER, $this->m_room_players[$key]['uid']);
                            unset($data);
                        }
                        if ($this->m_sysPhase == ConstConfig::SYSTEMPHASE_SET_OVER)
                        {
                            //发消息
                            $this->_send_cmd('s_ready', array('base_player_count'=>$this->m_rule->player_count, 'm_room_players'=>$this->m_room_players, 'm_ready'=>$this->m_ready, 'm_nSetCount'=>$this->m_nSetCount, 'm_wTotalScore'=>$this->m_wTotalScore), Game_cmd::SCO_SINGLE_PLAYER, $this->m_room_players[$key]['uid']);
                            unset($data);
                        }
	...
	public function c_ready($fd, $params)
	{
		...
		do {
			...
			if($ready_count == $this->m_rule->player_count )
			{
                ...
                for ($i=0; $i < $this->m_rule->player_count ; $i++)
                {
                    $this->_send_cmd('s_start',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
                }
                unset($data);
				$this->on_start_game();
	...
	public function HandleChoosePeng($chair)
	{
        ...
        $data = array();
        $data['m_sPeng']['chair'] = $chair;
        $data['m_sPeng']['card'] = $temp_card;
        $data['m_sPeng']['outedcard_chair'] = $this->m_sStandCard[$chair]->who_give_me[$stand_count];
        //$data['m_nTableCards'] = $this->m_nTableCards;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $this->_send_cmd('s_peng',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
        }
        unset($data);
	...
	public function HandleChooseEat($chair,$eat_num)
	{
        ...
        $data = array();
        $data['m_sEat']['chair'] = $chair;
        $data['m_sEat']['card'] = $temp_card;
        $data['m_sEat']['outedcard_chair'] = $this->m_sStandCard[$chair]->who_give_me[$stand_count];
        $data['m_sEat']['first_card'] = $this->m_sStandCard[$chair]->first_card[$stand_count];
        //$data['m_nTableCards'] = $this->m_nTableCards;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $this->_send_cmd('s_eat',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
        }
        unset($data);
		return true;
	}
	...
	public function HandleChooseAnGang($chair, $gang_card)
	{
        ...
        $data = array();
        $data['m_sAngang']['chair'] = $chair;
        $data['m_sAngang']['card'] = $temp_card;
        $data['m_sAngang']['outedcard_chair'] = $this->m_sStandCard[$chair]->who_give_me[$stand_count];
        //$data['m_nTableCards'] = $this->m_nTableCards;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $this->_send_cmd('s_angang',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
        }
        unset($data);
	}
	...
	public function HandleChooseZhiGang($chair)
	{
        ...
        unset($data);
        $data['m_sZhigang']['chair'] = $chair;
        $data['m_sZhigang']['card'] = $temp_card;
        $data['m_sZhigang']['outedcard_chair'] = $this->m_sStandCard[$chair]->who_give_me[$stand_count];
        //$data['m_nTableCards'] = $this->m_nTableCards;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $this->_send_cmd('s_zhigang',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
        }
	}
	...
	public function HandleChooseWanGang($chair, $gane_card)
	{
        ...
        $data = array();
        $data['m_sWangang']['chair'] = $chair;
        $data['m_sWangang']['card'] = $temp_card;
        //$data['m_nTableCards'] = $this->m_nTableCards;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $this->_send_cmd('s_wangang',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
        }
        unset($data);
	...
	public function HandleOutCard($chair, $is_14 = false, $out_card = 0, $is_ting = 1)		
	{
        ...
        //发送动作
        $data = array();
        $data['m_sOutedCard']['chair'] = $chair;
        $data['m_sOutedCard']['card'] = $this->m_sOutedCard->card;
        $data['m_sOutedCard']['m_nTableCards'] = $this->m_nTableCards;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            if ($i==$chair)
            {
            ...
            {
                $this->_send_cmd('s_discard',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
            }
        }
        unset($data);
        //初始化
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $data['m_sPlayer_card'][]=array();
        }
        $data['m_sOutedCard']['chair'] = $chair;
        $data['m_sOutedCard']['card'] = $this->m_sOutedCard->card;
        $data['m_sPlayer_card'][$chair]=$this->_m_sPlayer_card_into_array($chair);
        //$data['m_nTableCards'] = $this->m_nTableCards;
        $this->_send_cmd('s_discard',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$chair]['uid']);
        unset($data);
    ...
    public function OnGetChairScene_test($chair, $is_more=false)
    {
        ...
        if ($this->m_sysPhase == ConstConfig::SYSTEMPHASE_THINKING_OUT_CARD)
        {
            ...
            {
                $data['m_sDraw']['card']=$this->m_sPlayer[$chair]->card_taken_now;
            }*/
            //初始化
            for ($i=0; $i < $this->m_rule->player_count ; $i++)
            {
            ...
            for ($i=0; $i<$this->m_rule->player_count; $i++)                                         // 玩家手持牌长度
            {
                ...
                {
                    $data['m_sPlayer'][$i] = $this->m_sPlayer[$i];
                    $data['m_sPlayer_card'][$i] = $this->_m_sPlayer_card_into_array($i);
                    $data['m_bChooseBuf'] = $this->m_bChooseBuf[$i];			 //命令缓冲
                    $data['m_nHuGiveUp'] = $this->m_nHuGiveUp[$i];
                    $data['m_only_out_card'] = $this->m_only_out_card[$i];
                }
        ...
        if ($this->m_sysPhase == ConstConfig::SYSTEMPHASE_CHOOSING )
        {
            ...
            if ($chair==$this->m_chairCurrentPlayer)
            {
                $m_nTableCards_temp = $this->m_nTableCards;
                unset($m_nTableCards_temp[$this->m_chairCurrentPlayer][count($m_nTableCards_temp[$this->m_chairCurrentPlayer])-1]);
                $data['m_nTableCards'] = $m_nTableCards_temp;
            }else
            {
                $data['m_nTableCards'] = $this->m_nTableCards;	// 玩家桌面牌
            }
	...
	public function DealAllCardEx()
	{
        ...
        {
            $data['m_sPlayer_card'][$i]=$this->_m_sPlayer_card_into_array($i);
            $this->_send_cmd('s_draw_all',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
            $data['m_sPlayer_card'][$i]=array();
        }
        ...
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $this->_send_cmd('s_fanhun',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
        }
        unset($data);
        $data = array();
        $data['m_sDraw']['chair'] = $this->m_nChairBanker;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            if ($i==$this->m_nChairBanker)
            {
                $data['m_sDraw']['card'] =  $this->m_sPlayer[$this->m_nChairBanker]->card_taken_now;
                $this->_send_cmd('s_draw',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
                unset($data['m_sDraw']['card']);
            }
            else
            {
	...
	public function DealCard($chair)
	{
		...
		//发送抓牌动作
        $data = array();
        $data['m_sDraw']['chair'] = $chair;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            if ($i==$chair)
            {
                $data['m_sDraw']['card'] =  $the_card;
                $this->_send_cmd('s_draw',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
                unset($data['m_sDraw']['card']);
            }
            else
    ...
    public function on_start_game()
    {
        ...
        //发送动作的数据
        $data = array();
        $data['m_nChairBanker'] = $this->m_nChairBanker;
        $data['m_dice'] = $this->m_dice;
        for ($i=0; $i < $this->m_rule->player_count ; $i++)
        {
            $this->_send_cmd('s_dealer',$data, Game_cmd::SCO_SINGLE_PLAYER , $this->m_room_players[$i]['uid']);
        }
        unset($data);
    ...
    public function start_la_zhuang()
    {
        ...
        for ($i = 0; $i < $this->m_rule->player_count ; ++$i)
        {
            ...
            $data['m_own_lazhuang'] = $this->m_own_lazhuang;
            //发消息
            $this->_send_cmd('s_lazhuang', $data, Game_cmd::SCO_SINGLE_PLAYER, $this->m_room_players[$i]['uid']);
        }
        unset($data);
    }
    ...
    public function _m_sPlayer_card_into_array($chair)
    {
        $data = array();
        for ($i = ConstConfig::PAI_TYPE_WAN; $i <= ConstConfig::PAI_TYPE_FENG ; $i++)
        {
