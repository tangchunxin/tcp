...
class GameCangZhou extends BaseGame
{
	...
	public function judge_fan($chair)
	{
        ...
        if ($hu_type==self::HU_TYPE_PINGHU
            &&
            (in_array(self::ATTACHED_HU_QINGYISE,$this->m_HuCurt[$chair]->method)
            ||in_array(self::ATTACHED_HU_ZIYISE,$this->m_HuCurt[$chair]->method)
            ||in_array(self::ATTACHED_HU_YITIAOLONG,$this->m_HuCurt[$chair]->method)
            ||in_array(self::ATTACHED_HU_BIAN,$this->m_HuCurt[$chair]->method)
            ||in_array(self::ATTACHED_HU_ZUAN,$this->m_HuCurt[$chair]->method)
            ||in_array(self::ATTACHED_HU_ZA,$this->m_HuCurt[$chair]->method)
            ||in_array(self::ATTACHED_HU_WUKUI,$this->m_HuCurt[$chair]->method)
            ||in_array(self::ATTACHED_HU_DIAOWUWAN,$this->m_HuCurt[$chair]->method))
        )
        {
            $tmp_hu_desc = '(';
        }
	...
	public function HandleChoosePeng($chair, $za = 0)
	{
		...
		{
			var_dump('碰：出牌结构：'.$this->m_sOutedCard.'桌面牌：'.$this->m_nTableCards);
		}
		var_dump('碰：出牌结构：');
    ...
    public function WashCard()
    {
        if($this->m_rule->is_feng)
        {
            ...
            if(!empty(ConstConfig::$ALL_CARD))
            {
                $this->m_nCardBuf = ConstConfig::$ALL_CARD;
            }
        }
        else
        {
