...
class BaseGame
{
    ...
    public $m_client_ip = array();                      // 公会房代理的玩家id
	...
	public function _send_act($cmd, $chair, $card=0, $hu_type=0)
	{
		$this->_send_cmd('s_act', array('cmd'=>$cmd, 'chair'=>$chair, 'card'=>$card, 'hu_type'=>$hu_type), Game_cmd::SCO_ALL_PLAYER);
	}
    ...
    public function _cancle_game()
    {
        ...
        if(!empty($this->m_rule->cancle_clocker))
        {
	        ...
	        {
	            if($yes_count >= $this->m_rule->player_count - 1 && $yes_count>1)
	            {
	                $this->m_room_state = ConstConfig::ROOM_STATE_OVER;
	                $is_cancle = 1;
	            }
	            else if($no_count >= 2 || $no_count>=($this->m_rule->player_count - 1))
	            {
	                for($i = 0 ; $i < $this->m_rule->player_count; $i++ )
	                {
	                    $this->m_cancle[$i] = 0;
	...
	public function _set_record_game($act, $param_1 = 0, $param_2 = 0, $param_3 = 0, $param_4 = 0)
	{
		...
		if(in_array($act, [ConstConfig::RECORD_CHI, ConstConfig::RECORD_PENG, ConstConfig::RECORD_ZHIGANG
			...
			, ConstConfig::RECORD_PASS, ConstConfig::RECORD_KOU_TING, ConstConfig::RECORD_SHAIPAI
            , ConstConfig::RECORD_TEHU_QIANGGANG,ConstConfig::RECORD_TING,ConstConfig::RECORD_DIAOYU
			]
			))
		{
			if(is_array($param_1))
    ...
    public function record_game_log($is_room_over = false, $is_log = false)
    {
		...
		BaseFunction::web_curl(array(
			...
			'game_table_info'=> json_encode($game_table_info, JSON_UNESCAPED_UNICODE), 
			'agent_uid' => $agent_uid
		));
    }
    ...
    public function diamond_settlement(&$currency_change_group, $is_room_over)
    {
    	if (isset($this->m_rule->pay_type))
        {
            if ($this->m_rule->pay_type == 0)
            {
                ...
                {
                	$currency = $this->_getNeedCurrency();
                    if (!empty($currency)) 
                    {
                    	$currency = -$currency;
            ...
            if ($this->m_rule->pay_type == 1)
            {
                ...
                {
                	$currency = $this->_getNeedCurrency();
                    $currency = -(ceil($currency/$this->m_rule->player_count));
                    for($i = 0; $i < $this->m_rule->player_count; $i++)
                    {
            ...
            if ($this->m_rule->pay_type == 2 && $is_room_over == 1 )
            {
                ...
                $currency = $this->_getNeedCurrency();
                $currency = -(ceil($currency/$winner_count));
    ...
    protected function _getNeedCurrency()
    {
    	$result = Room::$get_conf;
    	$currency_tmp = 0;
	...
	//删除桌面牌
    protected function _deleteLastTableCard()
    {
		...
		{
			var_dump('碰：出牌结构：');
			var_dump($this->m_sOutedCard);
			var_dump('桌面牌：');
			var_dump($this->m_nTableCards);
		}
    }
}