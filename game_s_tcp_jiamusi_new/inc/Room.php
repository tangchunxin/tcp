...
class She_te
{
    public $gang_chair = 255;//杠牌的chair
    public $she_te_chair = 255;//设特chair
    public $mark = false;//设特标记
    public function clear()
    {
        $this->gang_chair = 255;
        $this->she_te_chair = 255;
        $this->mark = false;
    }
    public function __construct()
    {
...
class RuleHuaNan
{
    public $game_type;          //佳木斯桦南161
    public $player_count;       //玩家人数 2 3 4
    public $set_num;            //房间局数 2圈 4圈 8圈
    public $min_fan;            //胡牌最小番 0
    public $top_fan;            //不封顶 0
    public $is_circle;          //按圈打几圈

    public $pay_type;           //付费方式: (0:房主付费, 1:AA付费, 2:大赢家付费)
    public $cancle_clocker;     //是否有倒计时 0 否 1 是

    public $is_qixiaodui;       //是否可胡七小对 0 否 1 是(默认1)
    public $is_piaoteyibianda;  //是否可飘胡一边大 0 否 1 是(默认0)

    public $allow_louhu;        //没有过手胡
...
class RuleFuJin
{
    ...
    public $is_fanhui;//上下翻会 0  上下翻会带本身 1  上下翻会带本身和红中 2  (默认0)

    public $is_piaohu;//是否可胡飘胡 0 否 1 是(默认1)
    public $is_xiadan;//是否下蛋 0 否 1 是(默认1)
    public $is_diaoyu;//是否钓鱼 0 否 1 是(默认1)
    public $is_fanhun;//是否显示翻混的那张牌
...
class RuleKuaiTing
{
    ...
    public $is_jiabao;//是否带假宝 0 否 1 是(默认0)
    public $is_hongzhongfei;//是否红中满天飞 0 否 1 是(默认1)
    public $is_guadafeng;//是否刮大风 0 否 1 是(默认1)
    public $is_lou;//是否漏 0 否 1 是(默认1)

    public $is_menqing;//是否门清 0 否 1 是(默认0)
    public $is_qingyise;//是否清一色 0 否 1 是(默认1)
    public $is_qixiaodui;//是否七小对 0 否 1 是(默认1)
    public $is_piaohu;//是否飘胡 0 否 1 是(默认1)
...
class Room
{
	...
	public static $game_type = array(
        '160' => 'gf\inc\GameFuJin'
        ,'161' => 'gf\inc\GameHuaNan'
        ,'162' => 'gf\inc\GameKuaiTing'
   ...
   "\"out_card\"",
	...
	public static function c_bind($serv, $fd, $params)
	{
		...
		do {
			if( empty($params['rid']) || empty($params['rid_key']))
			{
				...
				var_dump(__LINE__);var_dump($params);
			}
			...
			{	
				var_dump($fd_info);var_dump(__LINE__);var_dump($params);
	...
	public static function c_msg($serv, $fd, $params, $act)
	{
		...
		if (!empty($return_send['code']) && $act != 'c_get_room') 
		{
