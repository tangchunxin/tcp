...
class BaseGame
{
        ...
        do {
            ...
            foreach ($this->m_room_players as $key => $room_user)
            {
                if($room_user['uid'] == $params['uid'])
                {
                    ...
                    if(empty($this->m_sOutedCard->card) || $this->m_sOutedCard->chair == $key || 2 > $this->_list_find($key,$this->m_sOutedCard->card) || (!empty($this->m_rule->is_minglou) && !empty($this->m_minglou[$key])))
                    {
                        $this->c_cancle_choice($fd, $params);
	...
	public function _is_kanhun($chair, $type, $insert_item)
	{
		$return = false;
		$bType32 = false;
		$tmp_hu_data = &ConstConfig::$hu_data;
		// if(ConstConfig::PAI_TYPE_FENG == $fanhun_type)
		// {
		// 	$tmp_hu_data = &ConstConfig::$hu_data_feng;
		// }
		
		if(($this->m_HuCurt[$chair]->card != $this->m_hun_card) 
			|| ($this->m_sPlayer[$chair]->card[$type][$insert_item-1] == 0 || $this->m_sPlayer[$chair]->card[$type][$insert_item] == 0 || $this->m_sPlayer[$chair]->card[$type][$insert_item+1] == 0)
		)
		{
			return $return;
		}
		$insert_card = $this->_get_card_index($type, $insert_item);
		$this->_list_delete($chair,$insert_card -1);
		$this->_list_delete($chair,$insert_card);
		$this->_list_delete($chair,$insert_card +1);
		...
		$this->_list_insert($chair,$insert_card -1);
		$this->_list_insert($chair,$insert_card);
		$this->_list_insert($chair,$insert_card +1);

		if($bType32 && $this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)
		{
			$return = true;
		}
		return $return;
