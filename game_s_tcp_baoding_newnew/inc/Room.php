...
class Qiang_gang
{
	...
	public function init_data($mark, $card, $chair, $type = 0)
	{
		$this->mark = $mark;
		$this->card = $card;
		$this->chair = $chair;
...
class RuleBaoDing
{
	...
	public $cancle_clocker;		//解散倒计时 0 不带 1 带
	public $pay_type;			//支付方式 0 房主 1 AA 2 大赢家
...
class RuleAnxin
{
	...
	public $is_za_hu_mian_gang;	//砸胡免杠 0 否 1 是
	public $is_ganghua_fan;		//杠上花加番 0 否 1 是
	public $is_qidui_fan; 		//七对加番  0 否 1 是
	public $is_wangang_1_lose; 	//弯杠点碰者1人给分  0 否 1 是
	...
	public $is_zhuowukui_fan;	//是否捉五魁 0 否 1 是
	public $is_zhuang_fan;		//是否带庄闲 0 否 1 是
	public $is_chipai;			//是否带吃牌 0 否 1 是
	public $is_tiandi_hu_fan;	//是否带天地胡 0 否 1 是

	public $is_kou_card;		//是否带扣牌 0 否 1 是
	public $is_kou_dajiang;		//是否扣大将 0 否 1 是
	public $cancle_clocker;		//解散倒计时 0 不带 1 带
	public $pay_type;			//支付方式 0 房主 1 AA 2 大赢家
...
class RuleDingxing
{
    ...
    public $is_kou_card;	    // 是否扣四 0 否 1 是
    public $is_kou_dajiang;	    // 是否扣大将 0 否 1 是
    public $is_dianpao_bao;     // 是否点炮大包 0 点炮一家出 1 点炮大包庄 2 点炮三家出 999 一条龙上家大包(后台处理)
    public $cancle_clocker;		//解散倒计时 0 不带 1 带
	public $pay_type;			//支付方式 0 房主 1 AA 2 大赢家
...
class RuleMancheng
{
    ...
    public $is_yipao_duoxiang;	// 是否一炮多响 0 否 1 是
   	public $is_genzhuang; 	 	// 跟庄 0 否 1 是
   	public $is_zhuang_fan;		// 庄闲 0 否 1 是
    ...
    public $is_dianpao_bao;     // 是否点炮大包 0 点炮一家出 1 点炮大包庄 2 点炮三家出 999 一条龙上家大包(后台处理)
    public $cancle_clocker;		// 解散计时器 0 否 1 是
	public $pay_type;			// 支付方式 0 房主付费 1 AA付费 2大赢家付费
	public $qg_is_zimo;			// 是否抢杠算自摸 0 否 1 是
    public $is_da8zhang;		// 是否打八张 0 否 1 是
    public $is_wangang_1_lose;	// 是否弯杠点碰者出 0 否 1 是
    public $is_za_hu_mian_gang;	// 是否砸胡免杠 0 否 1 是
    public $is_shisanyao_fan;	// 是否带十三幺 0 否 1 是
    public $is_kou13;			// 是否扣十三张 0 否 1 是
...
class RuleGaobeidian
{
    public $game_type;      //游戏类型
    public $player_count;   //玩家人数: (2:2人, 3:3人, 4:4人)
    public $set_num;        //房间局数: (4:4局2钻, 8:8局3钻, 16:16局4钻)
    public $is_circle;      //圈数：(1:1圈2钻,2:2圈3钻,4:4圈6钻)
    public $min_fan;        //胡牌最小番

    public $top_fan;        //封顶番数: (40：40,80：80,120：120,0：不封顶)
    public $is_minglou;     //晒牌: (0:否,1:是)
    public $is_feng;        //带风牌: (0:否,1:是)
    public $is_yipao_duoxiang;	//一炮多响: (0:否,1:是)
    public $is_fanhun;      //翻混儿: (0:否,1:是)

    public $is_chipai;      //可吃牌: (0:否,1:是)
    public $is_ganghua_fan;	//杠上开花: (0:否,1:是)
    public $is_qidui_fan;	//七对子: (0:否,1:是)
    public $is_dianpao_bao;	//点炮选项(点炮一家出：0,点炮大包：1,点炮三家出：2)
    public $is_kou_card;	//扣牌: (0:否,1:是)

    public $is_wangang_1_lose;	//弯杠点碰者出: (0:否,1:是)
    public $is_zhuang_fan;	//带庄闲: (0:否,1:是)
    public $cancle_clocker;	//解散倒计时: (0:否,1:是)
    public $pay_type;       //支付方式: (0:房主付费,1:AA付费,2:大赢家付费)
...
class Room
{
	...
	public static $game_type = array(
	    '121' => 'gf\inc\GameXushui'
	    , '122' => 'gf\inc\GameBaoding'
	    , '123' => 'gf\inc\GameAnxin'
        , '124' => 'gf\inc\GameDingxing'
        , '125' => 'gf\inc\GameXiongxian'
        , '126' => 'gf\inc\GameMancheng'
        , '127' => 'gf\inc\GameGaoyang'
        , '128' => 'gf\inc\GameGaobeidian'
        , '129' => 'gf\inc\GameRongcheng'
