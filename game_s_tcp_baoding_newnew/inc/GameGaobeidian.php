...
class GameGaobeidian extends BaseGame
{
	...
	const ATTACHED_HU_SUHU = 63;				// 素胡
	const ATTACHED_HU_KANHUN = 64;				// 坎混
	const ATTACHED_HU_SHAI = 65;				// 晒牌
	...
	public static $attached_hu_arr = array(
		...
		,self::ATTACHED_HU_KANHUN=>array(self::ATTACHED_HU_KANHUN, 4, '坎混')
		,self::ATTACHED_HU_SHAI=>array(self::ATTACHED_HU_SHAI, 2, '晒牌')
	);	
	...
	public function InitDataSub()
    {
        ...
        $this->m_minglou = array(0,0,0,0); 		//晒牌结构
        $this->countGang = 0;
    }
    ...
    public function _open_room_sub($params)
    {
        ...
        $params['rule']['game_type'] = isset($params['rule']['game_type']) ? $params['rule']['game_type']: 128;
        ...
        $params['rule']['is_circle'] = isset($params['rule']['is_circle']) ? $params['rule']['is_circle']: 2;
        $params['rule']['min_fan'] = isset($params['rule']['min_fan']) ? $params['rule']['min_fan']: 0;

        $params['rule']['top_fan'] = isset($params['rule']['top_fan']) ? $params['rule']['top_fan']: 0;
        $params['rule']['is_minglou'] = isset($params['rule']['is_minglou']) ? $params['rule']['is_minglou']: 1;
        ...
        $params['rule']['is_chipai'] = isset($params['rule']['is_chipai']) ? $params['rule']['is_chipai']: 0;
        ...
        $params['rule']['is_kou_card'] = isset($params['rule']['is_kou_card']) ? $params['rule']['is_kou_card']: 0;

        $params['rule']['is_wangang_1_lose'] = isset($params['rule']['is_wangang_1_lose']) ? $params['rule']['is_wangang_1_lose']: 0;
        $params['rule']['is_zhuang_fan'] = isset($params['rule']['is_zhuang_fan']) ? $params['rule']['is_zhuang_fan']: 1;
				...
				if($room_user['uid'] == $params['uid'])
				{
					...
					if($this->m_minglou[$key] == 0)
					{
						...
						if(!empty($params['minglou']) && !empty($this->m_rule->is_minglou))
						{
							...
							//状态变化发消息
							$this->_send_act('s_shaipai', $key);
							$this->_set_record_game(ConstConfig::RECORD_SHAIPAI, $key);
	...
	public function judge_hu($chair, $is_fanhun = false)
	{
		...
		$is_kanhun = false;
		$is_suhu = false;
		$hu_type = $this->judge_hu_type_fanhun($chair, $is_suhu, $is_kanhun, $is_fanhun);
		...
		if ($is_kanhun)
		{
			$this->m_HuCurt[$chair]->add_hu(self::ATTACHED_HU_KANHUN);
		}
		//素胡
		if ($is_suhu)
		{
		...
		//明搂加番
		if($this->m_minglou[$chair] && !empty($this->m_rule->is_minglou))
		{
			$this->m_HuCurt[$chair]->add_hu(self::ATTACHED_HU_SHAI);			
		}
	...
	//判断翻混 
	public function judge_hu_type_fanhun($chair, &$is_suhu, &$is_kanhun, $is_fanhun = false)
	{
		$fanhun_num = 0;
		$fanhun_type = 255;
		if($this->m_hun_card)
		...
		else
		{
			...
			//混杠
			if ($this->m_sPlayer[$chair]->card[$fanhun_type][$fanhun_card] == 4 && $this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO && $this->m_minglou[$chair] == 0)
			{
				return self::HU_TYPE_HUNGANG;
			}
						...
						foreach ($tmp_replace_fanhun as $type => $num)
						{
							...
							foreach ($tmp_hu_data_insert[$num] as $insert_arr)
							{
								...
								if(isset($tmp_hu_data[$key]) && ($tmp_hu_data[$key] & 1) == 1)
								{
									...
									if ($fanhun_num == 1 && $num == 1 && ConstConfig::PAI_TYPE_FENG != $type && !empty($this->m_minglou[$chair]))
									{
										$is_kanhun = $this->_is_kanhun($chair, $type, $insert_item);
									}

									// if ($type == $fanhun_type)
									// {
									// 	$is_kanhun = $this->_is_kanhun($chair, $fanhun_type, $fanhun_card);
									// }
									foreach ($insert_arr as $insert_item)
									{
										$this->m_sPlayer[$chair]->card[$type][$insert_item] -= 1;
	...
	public function OnGetChairScene($chair, $is_more=false)
	{
		...
		$data['m_minglou'] = $this->m_minglou;  	//晒牌
	...
	public function ScoreOneHuCal($chair, &$lost_chair)  
	{
		...
		else if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_CHI_PAO)
		{
			...
			else if(empty($this->m_rule->is_dianpao_bao))
			{
				...
				// 对晒加倍
				if (!empty($this->m_minglou[$lost_chair]))
				{
					$wWinScore *= 2;
				}
	...
	public function WriteScore()      
	{
		...
		for($i = 0; $i < $this->m_rule->player_count; $i++)
		{
			...
			if($this->m_wSetLoseScore[$i])
			{
				...
				if (!empty($this->m_rule->is_minglou) && !empty($this->m_minglou[$i]))
				{
					if ($is_huchair_shai)
					{
						$this->m_hu_desc[$i] .= '对晒 ';
	...
	public function _list_insert_sub(&$arr, $card)
	{
	...
	public function _shai_arr($chair, $is_14, $out_card)
	{
		//接收数据
		// $this->m_sOutedCard->chair = $chair;
		// $pos  = $is_14;
