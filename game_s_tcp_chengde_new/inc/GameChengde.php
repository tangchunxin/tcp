...
class GameChengde
{
	...
	const ATTACHED_HU_QINGYISE = 65; 			//清一色
	const ATTACHED_HU_YITIAOLONG = 66; 			//一条龙
	const ATTACHED_HU_HAIDI = 67; 				//海底捞月 默认
	const ATTACHED_HU_ZIMOFAN = 68; 			//自摸     默认
	const ATTACHED_HU_SUHU = 69;               // 素胡
	...
	,self::HU_TYPE_HAOHUA_QIDUI=>[self::HU_TYPE_HAOHUA_QIDUI, 10, '豪华七对']
	);

	public static $attached_hu_arr = array(
	self::ATTACHED_HU_TIANHU=>[self::ATTACHED_HU_TIANHU, 4, '天胡']
		...
		do {
			...
			foreach ($this->m_room_players as $key => $room_user)
			{
				if($room_user['uid'] == $params['uid'])
				{
					...
					if(empty($this->m_sOutedCard->card) || $this->m_sOutedCard->chair == $key || $this->m_sOutedCard->chair != $this->_anti_clock($key,-1) || empty($this->m_rule->is_chipai))
					{
						$this->c_cancle_choice($fd, $params);
	...
	public function judge_hu($chair, $is_fanhun = false)
	{
		...
		$is_yitiaolong = false;
		$is_suhu = false;
		$hu_type = $this->judge_hu_type_fanhun($chair, $is_qingyise, $is_yitiaolong, $is_suhu, $is_fanhun);
	...
	//判断翻混
	// public function judge_hu_type_fanhun($chair, &$is_qingyise, &$is_yitiaolong, &$is_suhu, $is_fanhun = false)
	// {
	// 	$is_suhu = false;
	// 	$num_fanhun = 0;
	...
	// 	if($this->m_hun_card)
	// 	{
	// 		$num_fanhun = $this->_list_find($chair, $this->m_hun_card);	//手牌混牌个数
	// 		$fanhun_type = $this->_get_card_type($this->m_hun_card);       //混牌牌型
	// 		$fanhun_card = $this->m_hun_card%16;
	// 	}

	// 	if($is_fanhun)//打出的牌是否为混牌
	// 	{
	// 		$num_fanhun = $num_fanhun - 1;
	// 	}

	// 	if(0 == $this->m_rule->is_fanhun || 0 >= $num_fanhun)	//规则不带混 或者 手牌无混牌
	// 	{
	// 		if(!empty($this->m_rule->is_fanhun))
	// 		{
	// 			$is_suhu = true;
	// 		}
	// 		return $this->judge_hu_type($chair, $is_qingyise, $is_yitiaolong);
	// 	}
	// 	else
	// 	{
	...
	// 			//还原手牌中的翻混
	// 			$this->m_sPlayer[$chair]->card[$fanhun_type][$fanhun_card] += $num_fanhun;	
	...
	// 				for($i=ConstConfig::PAI_TYPE_WAN ; $i<=ConstConfig::PAI_TYPE_FENG ; $i++)
	// 				{
	// 					if(0 == $this->m_sPlayer[$chair]->card[$i][0] || (0 == ($this->m_sPlayer[$chair]->card[$i][0] - $num_fanhun) && $i == $fanhun_type ))
	// 					{
	// 						continue;
	// 					}
	// 					else
	...
	// 				if($need_fanhun <= $num_fanhun)
	// 				{
	// 					$hu_qidui = true;
	// 					$gen_count_num += intval(($num_fanhun - $need_fanhun)/2);
	// 				}
				
	// 				//其他影响胡七对的规则判定，缺门，打八张等等
	// 			}
	...
	// 			|| (0 == $this->m_sPlayer[$chair]->card[$i][0]-$num_fanhun && $i == $fanhun_type )
	// 			)
	// 			{
	// 				continue;
	// 			}
	...
	// 				|| (0 == $this->m_sPlayer[$chair]->card[$j][0]-$num_fanhun && $j == $fanhun_type )
	// 				)
	// 				{
	// 					continue;
	// 				}
	...
	// 							if(isset(ConstConfig::$hu_data[$key]) && (ConstConfig::$hu_data[$key] & 1) == 1)
	// 							{
	// 								if($this->m_rule->is_yitiaolong_fan && (ConstConfig::$hu_data[$key] & 256) == 256 && !$is_yitiaolong_tmp)  //判断一条龙
	// 								{
	// 									$is_yitiaolong_tmp = true;
	// 								}
	// 							}
	// 						}
	// 						continue;
	...
	// 						}
	// 					}
	// 					if($this->m_rule->is_yitiaolong_fan && $is_yitiaolong_tmp && !$is_yitiaolong)
	// 					{
	// 						$is_yitiaolong = $is_yitiaolong_tmp;
	// 					}
	...
	public function judge_hu_type($chair, &$is_qingyise, &$is_yitiaolong)
	{
		...
		if($this->m_rule->is_feng && $i = ConstConfig::PAI_TYPE_FENG)
		{
			if(0 < $this->m_sPlayer[$chair]->card[$i][0])
			{
				...
				else
				{
					...
					$haohua_qidui_arr[] = intval($hu_list_val/4096);
		...
		$bHaoHuaQiDui = ( 0 < array_sum($haohua_qidui_arr));
		...
		//素胡
		// if(!empty($this->m_rule->is_suhu_fan))
		// {
		// 	$is_suhu = true;
		// }
	...
	public function judge_fan($chair)
	{
		...
		for($i=1; $i<$this->m_HuCurt[$chair]->count; $i++)
		{
			...
			{
				//素胡不加分     只算倍
				if($this->m_HuCurt[$chair]->method[$i] != self::ATTACHED_HU_SUHU)
				{
	...
	{
		if ($this->m_game_type == 241 && $this->m_bChairHu[$chair])	//未胡玩家发牌
		{
	...
	public function ScoreOneHuCal($chair, &$lost_chair)
	{
		...
		if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)
		{
			...
			for($i = 0; $i < $this->m_rule->player_count; $i++)
			{
				...
				if ($this->m_game_type== 241 && $this->m_sPlayer[$i]->state == ConstConfig::PLAYER_STATUS_BLOOD_HU)
				{
					continue;
				}
	...
	private function _cancle_game()
	{
		...
		{
			if($yes_count >= $this->m_rule->player_count && $flee_count < $yes_count)
			{
				$this->m_room_state = ConstConfig::ROOM_STATE_OVER;
				$is_cancle = 1;
			}
	...
	private function _is_yise($qing_arr, &$is_qingyise, &$is_ziyise)
	{
		...
		if(1 == count(array_unique($qing_arr)))
		{
			...
			if(!empty($this->m_rule->is_ziyise) && ConstConfig::PAI_TYPE_FENG == $qing_arr[0])
			{
				$is_ziyise = true;
			}
		}
