...
class GameTangshan
{
	...
	const ATTACHED_HU_ZHUANG = 71;              // 庄家     默认
	const ATTACHED_HU_BIAN = 72;          //边
	const ATTACHED_HU_KA = 73;          //卡
	const ATTACHED_HU_DIAO = 74;          //吊
	const ATTACHED_HU_ZIMOFAN = 75;             // 自摸     默认

	const ATTACHED_HU_GANGKAI = 76;             // 杠开
	const ATTACHED_HU_QIANGGANG = 77;           // 抢杠
	const ATTACHED_HU_QINGYISE = 78;            // 清一色
	const ATTACHED_HU_HUNYISE = 79;            // 清一色
	const ATTACHED_HU_YITIAOLONG = 80;          // 一条龙

	const ATTACHED_HU_ZHUOWUKUI= 81;          // 捉五魁
	const ATTACHED_HU_TIANHU= 82;          // 天胡
	const ATTACHED_HU_DIHU= 83;          // 地胡
	const ATTACHED_HU_SUHU = 84;               // 素胡
	const ATTACHED_HU_XIAOSA = 85;               // 潇洒
	//const ATTACHED_HU_BENHUILONG = 86;               // 本混龙
	...
	self::ATTACHED_HU_ZHUANG=>[self::ATTACHED_HU_ZHUANG, 2, '庄']
	...
	,self::ATTACHED_HU_QIANGGANG=>[self::ATTACHED_HU_QIANGGANG, 2, '抢杠胡']
	...
	//,self::ATTACHED_HU_BENHUILONG=>[self::ATTACHED_HU_BENHUILONG, 3, '本混龙']

	,self::ATTACHED_HU_ZHUOWUKUI=>[self::ATTACHED_HU_ZHUOWUKUI, 4, '捉五魁']
	,self::ATTACHED_HU_TIANHU=>[self::ATTACHED_HU_TIANHU, 2, '天胡']
	...
	public $m_is_ting_arr = array(); //是否听牌，能点炮胡
	...
	public function __construct($serv)
	{
		...
		$this->m_game_type = self::GAMETYPE;
	}
	...
	public function InitData($is_open = false)
	{
		...
		{
			$this->m_game_type = self::GAMETYPE;	//游戏类型，见http端协议
	...
	public function c_open_room($fd, $params)
	{
		...
		do {
			...
			//$params['rule']['top_fan'] = !isset($params['rule']['top_fan']) ? 128 : $params['rule']['top_fan'];
			$params['rule']['top_fan'] = 128;
			...
			$params['rule']['is_xiaosa'] = !isset($params['rule']['is_xiaosa']) ? 0 : $params['rule']['is_xiaosa'];
			...
			$params['rule']['is_biankadiao'] = !isset($params['rule']['is_biankadiao']) ? 1 : $params['rule']['is_biankadiao'];

			$params['rule']['is_qingyise_fan'] = !isset($params['rule']['is_qingyise_fan']) ? 1 : $params['rule']['is_qingyise_fan'];
			$params['rule']['is_hunyise_fan'] = !isset($params['rule']['is_hunyise_fan']) ? 1 : $params['rule']['is_hunyise_fan'];
	...
	public function judge_hu($chair, $is_fanhun = false)
	{
		...
		//混杠  必须是自摸   点炮不是
		$hu_type = $this->judge_hu_type_fanhun($chair, $is_qingyise, $is_yitiaolong, $is_suhu, $is_bian, $is_ka, $is_diao, $is_zhuowukui, $is_hunyise, $is_fanhun);
		...
		elseif($is_hunyise && $this->m_rule->is_hunyise_fan)
		{
		...
		//本混龙
		// if($is_benhuilong && $this->m_rule->is_yitiaolong_fan)
		// {
		// 	$this->m_HuCurt[$chair]->add_hu(self::ATTACHED_HU_BENHUILONG);
		// }
		...
		//庄闲
		if( ($this->m_nChairBanker == $chair) && $this->m_rule->is_zhuang_fan)
		{
			$this->m_HuCurt[$chair]->add_hu(self::ATTACHED_HU_ZHUANG);
		}
	...
	//判断翻混
	public function judge_hu_type_fanhun($chair, &$is_qingyise, &$is_yitiaolong, &$is_suhu, &$is_bian, &$is_ka, &$is_diao, &$is_zhuowukui, &$is_hunyise, $is_fanhun = false )
	{
		$is_quemen = true;
		$fanhun_num = 0;
		if($this->m_hun_card)
		...
		{
			return $this->judge_hu_type($chair, $is_qingyise, $is_yitiaolong, $is_suhu, $is_bian, $is_ka, $is_diao, $is_zhuowukui, $is_hunyise);
		}
		else
		{
			...
			if(!empty($this->m_rule->is_qidui_fan))
			{
				...
				else
				{
					...
					if($hu_qidui)
					{
						$this->_is_yise($qing_arr, $is_qingyise, $is_ziyise, $is_hunyise);
			...
			for($i=ConstConfig::PAI_TYPE_WAN ; $i<=ConstConfig::PAI_TYPE_FENG ; $i++)
			{
				...
				if($need_fanhun <= $fanhun_num)
				{
					...
					for($j=ConstConfig::PAI_TYPE_WAN ; $j<=ConstConfig::PAI_TYPE_FENG ; $j++)
					{
						...
						$is_hu_data = false;
						$pengpeng_arr = $pengpeng_arr_stand;
						//$max_type_hu_arr = array(0=>-1, 1=>$pengpeng_arr_stand, 0, 0, 0);
						...
						foreach ($tmp_replace_fanhun as $type => $num)
						{
							...
							//$insert_benhuilong = $max_type_hu_arr[4];//上一门的数据

							foreach ($tmp_hu_data_insert[$num] as $insert_arr)
							{
								//$tmp_max_hu_fan = 0;
								//$tmp_insert_benhuilong = false;
								foreach ($insert_arr as $insert_item)
								{
									$this->m_sPlayer[$chair]->card[$type][$insert_item] += 1;
									////////////
									// if($fanhun_type == $type && $insert_item == $fanhun_card)
									// {
									// 	$tmp_insert_benhuilong = true;
									// }
								}
								...
								if( isset($tmp_hu_data[$key]) && ($tmp_hu_data[$key] & 1) == 1)
								{
									...
									if($type_len >= 9)
									{
										...
										{
											$tmp_type_hu_num += 2;
											// if($tmp_insert_benhuilong || $insert_benhuilong)
											// {
											// 	$tmp_benhuilong = true;
											// 	$tmp_type_hu_num += 1;
											// }
									...
									if($insert_biankadiao || $tmp_judge_biankadiao)
									{
										...
										if(!$tmp_judge_biankadiao)//继承上次的边 卡 吊
										{
									...
									if($tmp_type_hu_num >= $max_type_hu_arr[0])
									{
										...
										//$max_type_hu_arr[4] = $tmp_benhuilong;
									}
						...
						if($max_type_hu_arr[0] > 0)
						{
							...
							// if(!empty($this->m_rule->is_yitiaolong_fan) && $max_type_hu_arr[4])
							// {
							// 	$tmp_max_hu += self::$attached_hu_arr[self::ATTACHED_HU_BENHUILONG][1];
							// }
							...
							if($tmp_max_hu > $max_hu[0])
							{
								...
								//$max_hu[4] = $max_type_hu_arr[4];//变卡吊
							}
						}
			...
			if($max_hu[0] > 0)
			{
				...
				if($max_hu[3] != self::HU_TYPE_FENGDING_TYPE_INVALID)
				{
					...
					{
						if($max_hu[3] == self::ATTACHED_HU_ZHUOWUKUI)
						{
							$is_zhuowukui = true;
						}
					}
	...
	//胡牌类型判断  没有混的情况
	public function judge_hu_type($chair, &$is_qingyise, &$is_yitiaolong, &$is_suhu, &$is_bian, &$is_ka, &$is_diao, &$is_zhuowukui, &$is_hunyise)
	{
		$jiang_arr = array();
		$qidui_arr = array();
		$qing_arr = array();
		...
		if(!empty($this->m_rule->is_biankadiao))
		{
			...
			else
			{
				...
				{
					if($this->_is_ka($chair, $type, false, false, $is_zhuowukui))
					{
						if(!$is_zhuowukui)
						{
							$is_ka = true;
	...
	public function judge_fan($chair)
	{
		...
		for($i=1; $i<$this->m_HuCurt[$chair]->count; $i++)
		{
			if(isset(self::$attached_hu_arr[$this->m_HuCurt[$chair]->method[$i]]))
			{
				...
				if($this->m_HuCurt[$chair]->method[$i] != self::ATTACHED_HU_ZHUANG)
				{
	...
	public function HandleHuZiMo($chair)			//处理自摸
	{
		...
		else
		{
			...
			if($this->ScoreOneHuCal($chair, $tmp_lost_chair))
			{
				...
				//if ($this->m_game_type == self::GAMETYPE)
				{
	...
	public function ScoreOneHuCal($chair, &$lost_chair)
	{
		...
		{
			//点炮大包
			if(!empty($this->m_rule->is_dianpao_bao) && $this->m_rule->is_dianpao_bao == 1)//点炮大包
			{
				for($i = 0; $i < $this->m_rule->player_count; $i++)
				{
			...
			else if(!empty($this->m_rule->is_dianpao_bao) && $this->m_rule->is_dianpao_bao == 2)  //点炮三家出
			{
				//点炮三家出
				for($i = 0; $i < $this->m_rule->player_count; $i++)
				{
			...
			else if(empty($this->m_rule->is_dianpao_bao)) //点炮一家出
			{
				$banker_fan = 1;
				if(!empty($this->m_rule->is_zhuang_fan) && ($this->m_nChairBanker == $chair || $this->m_nChairBanker == $lost_chair))
				{
	...
	//判断一色结果，通过引用返回
	private function _is_yise($qing_arr, &$is_qingyise, &$is_ziyise ,&$is_hunyise)
	{
		if(1 == count(array_unique($qing_arr)))
		{
			...
			else if(!empty($this->m_rule->is_hunyise_fan) && ConstConfig::PAI_TYPE_FENG == $qing_arr[0])
			{
				$is_hunyise = true;
			}
			else
		...
		elseif(2 == count(array_unique($qing_arr)))
		{
			//混一色  唐山麻将字一色 算 混一色
			if(!empty($this->m_rule->is_hunyise_fan) && in_array(ConstConfig::PAI_TYPE_FENG,$qing_arr))
			{
	            $is_hunyise = true;
			}
			else
	...
	public function HandleBianKaDiao($chair, $type , $insert_arr = array() , $is_fanhun = false, $is_boss_diao = false)
	{
		...
		if(($this->m_HuCurt[$chair]->card == $this->m_hun_card && $is_fanhun == false) || $type == $this->_get_card_type($this->m_HuCurt[$chair]->card))
		{
			if(!empty($this->m_rule->is_biankadiao))
			{
				...
				{
					if($this->_is_ka($chair, $type, $insert_arr, $is_fanhun ,$is_zhuowukui))
					{
						if($is_zhuowukui)
						{
							$return_type = self::ATTACHED_HU_ZHUOWUKUI;
						}
						else
						{
	...
	//卡
	private function  _is_ka($chair, $type, $insert_arr = array(), $is_fanhun = false, &$is_zhuowukui)
	{
		$return = false;
		//胡的牌
		$hu_card = $this->m_HuCurt[$chair]->card;
			...
			foreach ($insert_arr as $insert_item)
			{
				if($insert_item != 1 && $insert_item != 9)
				{
					...
					if($this->m_sPlayer[$chair]->card[$type][$insert_item] > 0 && $this->m_sPlayer[$chair]->card[$type][$tmp_ka4] > 0 && $this->m_sPlayer[$chair]->card[$type][$tmp_ka6] > 0)
					{
						...
						{
							if($insert_item == 5 && $type == ConstConfig::PAI_TYPE_WAN)
							{
								$is_zhuowukui = true;
								return true;
							}
		...
		else
		{
			if($hu_type == $type && $hu_type != ConstConfig::PAI_TYPE_FENG && ($hu_card_index != 1 && $hu_card_index != 9))
			{
				...
				if($this->m_sPlayer[$chair]->card[$type][$tmp_ka4] > 0 && $this->m_sPlayer[$chair]->card[$type][$tmp_ka6] > 0)
				{
					...
					{
						if($hu_card_index == 5 && $type == ConstConfig::PAI_TYPE_WAN)
						{
							$is_zhuowukui = true;
						}
						return true;
