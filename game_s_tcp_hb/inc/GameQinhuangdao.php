...
class GameQinhuangdao
{
	...
	public function InitData($is_open = false)
	{
		...
		{
			$this->m_game_type = 221;	//游戏类型，见http端协议
	...
	public function c_open_room($fd, $params)
	{
		...
		do {
			...
			$this->clear();
			$this->m_rule = new RuleQinhuangdao();
	...
	public function judge_fan($chair)
	{
		...
		//验证混杠  不算自摸的2分
		if($hu_type != self::HU_TYPE_HUNGANG && $this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO && ($fan_sum -  self::$attached_hu_arr[self::ATTACHED_HU_ZIMOFAN][1]) <= self::$hu_type_arr[self::HU_TYPE_HUNGANG][1])
		{
