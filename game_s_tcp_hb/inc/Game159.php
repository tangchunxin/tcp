...
class Game159
{
    ...
    public static $hu_type_arr = array(
        ...
        self::HU_TYPE_QIDUI => [self::HU_TYPE_QIDUI, 1, '七对'],
        self::HU_TYPE_SIHUNZI => [self::HU_TYPE_SIHUNZI, 1, '四混儿']
    );
    ...
    public $m_ZhuaNiaoCard = array();                // 抓鸟翻出的牌
    public $m_wZhuaniaoScore = array(0,0,0,0);       // 抓鸟分数
...
//        $this->m_hun_card = 0;	//混牌
        $this->m_ZhuaNiaoCard = array();
        $this->m_wZhuaniaoScore = array(0,0,0,0);
            ...
            if (!empty($params['uid'])) {
                foreach ($this->m_room_players as $key => $room_user) {
                    if ($room_user['uid'] == $params['uid']) {
                        ...
                        $cmd = new Game_cmd($this->m_room_id, 's_chat', array(
                            "type" => $params['type'],
                            "content" => $params['content'],
                            "chair" => $key,
                            "uid" => $params['uid']
                        ), Game_cmd::SCO_ALL_PLAYER);
                        $cmd->send($this->serv);
                        unset($cmd);
                    }
                }
            ...
            if (!empty($params['uid'])) {
                foreach ($this->m_room_players as $key => $room_user) {
                    if ($room_user['uid'] == $params['uid']) {
                        ...
                        $cmd = new Game_cmd($this->m_room_id, 's_sys_phase_change', $this->OnGetChairScene($key, true),
                            Game_cmd::SCO_SINGLE_PLAYER, $this->m_room_players[$key]['uid']);
                        $cmd->send($this->serv);
                        unset($cmd);
    ...
    public function c_open_room($fd, $params)
    {
        ...
        $itime = time();
        do {
            if (empty($params['rid']) || empty($params['uid']) || empty($params['game_type']) || empty($params['rule'])) {
                $return_send['code'] = 1;
                $return_send['text'] = '参数错误';
                $return_send['desc'] = __LINE__.__CLASS__;
                break;
            ...
            $this->m_rule->is_hongzhonghui = $params['rule']['is_hongzhonghui'];
            $this->m_rule->is_zhuang_fan = $params['rule']['is_zhuang_fan'];
            $this->m_rule->is_sihongzhong = $params['rule']['is_sihongzhong'];
            $this->m_rule->is_zhuaniao = $params['rule']['is_zhuaniao'];
    ...
    public function c_join_room($fd, $params)
    {
        ...
        do {
            ...
            if ($this->m_sysPhase != ConstConfig::SYSTEMPHASE_SET_OVER || (ConstConfig::ROOM_STATE_OPEN != $this->m_room_state && ConstConfig::ROOM_STATE_GAMEING != $this->m_room_state)) {
                ...
                if (empty($this->m_room_players) && ($itime - $this->m_start_time) > Room::$room_timeout) {
        ...
        if (0 == $return_send['code']) {
            ...
            $cmd = new Game_cmd($this->m_room_id, 's_join_room',
                array('m_room_players' => $this->m_room_players, 'm_ready' => $this->m_ready),
                Game_cmd::SCO_ALL_PLAYER);
            $cmd->send($this->serv);
            unset($cmd);
    ...
    public function c_ready($fd, $params)
    {
        ...
        do {
            ...
            if ($this->m_sysPhase != ConstConfig::SYSTEMPHASE_SET_OVER || (ConstConfig::ROOM_STATE_OPEN != $this->m_room_state && ConstConfig::ROOM_STATE_GAMEING != $this->m_room_state)) {
                $cmd = new Game_cmd($this->m_room_id, 's_sys_phase_change', $this->OnGetChairScene($u_key, true),
                    Game_cmd::SCO_SINGLE_PLAYER, $params['uid']);
                $cmd->send($this->serv);
                unset($cmd);
                $return_send['code'] = 2;
                $return_send['text'] = '房间状态错误';
            ...
            if (0 == $return_send['code']) {
                $this->handle_flee_play(true);    //更新断线用户
                $cmd = new Game_cmd($this->m_room_id, 's_ready', array(
                    'base_player_count' => $this->m_rule->player_count,
                    'm_room_players' => $this->m_room_players,
                    'm_ready' => $this->m_ready,
                    'm_nSetCount' => $this->m_nSetCount,
                    'm_wTotalScore' => $this->m_wTotalScore
                ), Game_cmd::SCO_ALL_PLAYER);
                $cmd->send($this->serv);
                unset($cmd);
            }
            ...
            foreach ($this->m_room_players as $key => $room_user) {
                if ($room_user['uid'] == $params['uid']) {
                    ...
                    if (4 != $this->_list_find($key, $params['gang_card'])
                        && !(($params['gang_card'] == $this->m_sPlayer[$key]->card_taken_now) && 3 == $this->_list_find($key,
                                $params['gang_card']))
                    ) {
                        $return_send['code'] = 5;
        ...
        do {
            ...
            foreach ($this->m_room_players as $key => $room_user) {
                if ($room_user['uid'] == $params['uid']) {
                    ...
                    if (!$have_wan_gang || ($params['gang_card'] != $this->m_sPlayer[$key]->card_taken_now && 0 == $this->_list_find($key,
                                $params['gang_card']))
                    ) {
                        $return_send['code'] = 5;
                        $return_send['text'] = '杠牌错误';
            ...
            foreach ($this->m_room_players as $key => $room_user) {
                if ($room_user['uid'] == $params['uid']) {
                    ...
                    for ($i = 0; $i < $this->m_rule->player_count; $i++) {
                        ...
                        if (self::is_hu_give_up($temp_card,
                                $this->m_nHuGiveUp[$last_chair]) || !$this->judge_hu($last_chair, $is_laizi)
                        ) {
        ...
        do {
            ...
            foreach ($this->m_room_players as $key => $room_user) {
                if ($room_user['uid'] == $params['uid']) {
                    ...
                    if (empty($this->m_sOutedCard->card) || $this->m_sOutedCard->chair == $key || 2 > $this->_list_find($key,
                            $this->m_sOutedCard->card)
                    ) {
                        $this->c_cancle_choice($fd, $params);
                        $return_send['code'] = 5;
        ...
        do {
            ...
            foreach ($this->m_room_players as $key => $room_user) {
                if ($room_user['uid'] == $params['uid']) {
                    ...
                    if (empty($this->m_sOutedCard->card) || $this->m_sOutedCard->chair == $key || 3 > $this->_list_find($key,
                            $this->m_sOutedCard->card)
                    ) {
                        $this->c_cancle_choice($fd, $params);
                        $return_send['code'] = 5;
    ...
    {
        //胡牌型
        $hu_type = $this->judge_hu_type_hongzhong($chair, $is_laizi);
    ...
    //判断红中癞子
    public function judge_hu_type_hongzhong($chair, $is_laizi = false)
    {
        $fanhun_num = $this->m_sPlayer[$chair]->card[ConstConfig::PAI_TYPE_FENG][5];	//手牌翻混个数
        $fanhun_type = ConstConfig::PAI_TYPE_FENG;        //翻混牌类型
        $fanhun_card = 5;       //翻混牌

        $fanhun_num = $is_laizi ? $fanhun_num - 1 : $fanhun_num;	//打出的牌是否为翻混

        if(0 == $this->m_rule->is_hongzhonghui || 0 >= $fanhun_num)	//规则混子 或者 手牌无混中
        {
        ...
        else
        {
            ...
            //四混儿直接胡
            if ($fanhun_num == 4 && ($this->m_rule->is_sihongzhong != 0) && ($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)) {
            ...
            {
                //去掉翻混
                $this->m_sPlayer[$chair]->card[$fanhun_type][$fanhun_card] = $is_laizi ? 1 : 0;
            ...
            for($i=ConstConfig::PAI_TYPE_WAN ; $i<=ConstConfig::PAI_TYPE_FENG ; $i++)
            {
                ...
                for($j=ConstConfig::PAI_TYPE_WAN ; $j<=ConstConfig::PAI_TYPE_FENG ; $j++)
                {
                    ...
                    else
                    {
                        if($j == $fanhun_type)
                        {
                            $this->m_sPlayer[$chair]->card[$fanhun_type][$fanhun_card] = $is_laizi ? 1 : 0;	//去掉翻混
                        }
    ...
    public function HandleOutCard($chair, $is_14 = false, $out_card = 0, $is_ting = 1)
    {
        ...
        if ($pos) {
            ...
            if ($temp_out_card) {    //若打出的是第1-13张牌, 要整理牌列表
        ...
        for ($i = 0; $i < $this->m_rule->player_count - 1; $i++) {
            ...
            $cmd = new Game_cmd($this->m_room_id, 's_sys_phase_change', $this->OnGetChairScene($chair_next),
                Game_cmd::SCO_SINGLE_PLAYER, $this->m_room_players[$chair_next]['uid']);
            $cmd->send($this->serv);
            unset($cmd);
        }

        $cmd = new Game_cmd($this->m_room_id, 's_sys_phase_change', $this->OnGetChairScene($chair),
            Game_cmd::SCO_SINGLE_PLAYER, $this->m_room_players[$chair]['uid']);
        $cmd->send($this->serv);
        unset($cmd);

        //其他用户无需操作(一个用户的时候 单人麻将发牌)
        if (!$bHaveCmd) {
            $next_chair = $chair;
            do {
                $next_chair = $this->_anti_clock($next_chair);
    ...
    public function OnGetChairScene($chair, $is_more = false)
    {
        ...
        if ($this->m_sysPhase == ConstConfig::SYSTEMPHASE_SET_OVER) {
            ...
            $data['m_ZhuaNiaoCard'] = $this->m_ZhuaNiaoCard;    //抓鸟牌
            $data['m_wZhuaniaoScore'] = $this->m_wZhuaniaoScore; // 抓鸟分数
    ...
    public function HandleSetOver()
    {
        ...
        if ($this->m_nEndReason == ConstConfig::END_REASON_HU) {
            ...
            if ($this->m_nEndReason == ConstConfig::END_REASON_NOCARD) {
                ...
                if ($this->m_nEndReason == ConstConfig::END_REASON_FLEE) {        //逃跑结算游戏
                    //逃跑牌局等待，不结算
                    //$this->CalcFleeScore();
                } else {
    ...
    public function CalcHuScore()
    {
        ...
        for ($i = 0; $i < $this->m_rule->player_count; $i++) {
            $this->m_Score[$i]->score = $this->m_wSetScore[$i] + $this->m_wSetLoseScore[$i] + $this->m_wGangScore[$i][$i] + $this->m_wZhuaniaoScore[$i];
    ...
    public function WriteScore()
    {
        for ($i = 0; $i < $this->m_rule->player_count; $i++) {
            ...
            if ($this->m_rule->is_zhuaniao) {
                if ($this->m_wZhuaniaoScore[$i] > 0) {
                    $this->m_hu_desc[$i] .= '抓鸟+' . $this->m_wZhuaniaoScore[$i] . ' ';
                } else {
                    $this->m_hu_desc[$i] .= '抓鸟' . $this->m_wZhuaniaoScore[$i] . ' ';
                }
            }
    ...
    public function on_start_game()
    {
        ...
        $this->_set_record_game(ConstConfig::RECORD_DEALER, $this->m_nChairBanker, 0, 0,
            intval(implode('', $this->m_dice)));
    ...
    {
        $cmd = new Game_cmd($this->m_room_id, 's_act', array('cmd' => $cmd, 'chair' => $chair, 'card' => $card),
            Game_cmd::SCO_ALL_PLAYER);
        $cmd->send($this->serv);
        unset($cmd);
    }
    ...
    private function catchBird()
    {
        ...
        if ($this->m_rule->is_zhuaniao) {
            ...
            $bird_pool = array(1, 5, 9, 17, 21, 25, 33, 37, 41, 53);
            for ($i = 1; $i <= $this->m_rule->is_zhuaniao; $i++) {

                //没牌了
                if (empty($this->m_nCardBuf[$this->m_nCountAllot])) {
                    break;
                } else {
                    $this->m_ZhuaNiaoCard[] = $this->m_nCardBuf[$this->m_nCountAllot];
            ...
            for ($j = 0; $j < $this->m_rule->player_count; $j++) {
                if ($j == $chair) {
                    ...
                    $this->m_wZhuaniaoScore[$j] -=$count_bird;
                    $this->m_wZhuaniaoScore[$chair] += $count_bird;
                }
            }

            $m_ZhuaNiaoCard = $this->m_ZhuaNiaoCard;
            $num1 = isset($m_ZhuaNiaoCard[0]) ? $m_ZhuaNiaoCard[0] : 0;
            $num2 = isset($m_ZhuaNiaoCard[1]) ? $m_ZhuaNiaoCard[1] : 0;
            $num3 = isset($m_ZhuaNiaoCard[2]) ? $m_ZhuaNiaoCard[2] : 0;
            $num4 = isset($m_ZhuaNiaoCard[3]) ? $m_ZhuaNiaoCard[3] : 0;
            $num5 = isset($m_ZhuaNiaoCard[4]) ? $m_ZhuaNiaoCard[4] : 0;
            $num6 = isset($m_ZhuaNiaoCard[5]) ? $m_ZhuaNiaoCard[5] : 0;
