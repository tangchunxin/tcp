...
class GameXingtaiTuidaohu
{
	...
	public function InitData($is_open = false)
	{
		...
		{
			$this->m_game_type = 213;	//游戏 1 四川血战到底 2 陕西麻将(陕北麻将)
	...
	public function c_open_room($fd, $params)
	{
		...
		do {
			...
			$this->m_rule = new RuleXingtaiTuidaohu();
	...
	public function ScoreOneHuCal($chair, &$lost_chair)  
	{
		...
		if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)
		{
			...
			for($i = 0; $i < $this->m_rule->player_count; $i++)
			{
				...
				if ($this->m_game_type== 213 && $this->m_sPlayer[$i]->state == ConstConfig::PLAYER_STATUS_BLOOD_HU)
				{
					continue;
				}
				...
				$wWinScore = 0;
				$wWinScore += ConstConfig::SCORE_BASE * 2 * $PerWinScore;  //赢的分 加 自摸翻翻
	...
	{
		if ($this->m_game_type == 213 && $this->m_bChairHu[$chair])	//未胡玩家发牌
		{
