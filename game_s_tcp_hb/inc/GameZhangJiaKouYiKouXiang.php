...
class GameZhangJiaKouYiKouXiang extends BaseGame
{
	const GAME_TYPE = 271;
	//－－－－－－－－－－－－－胡牌类型 －－－－－－－－－－－－－－－－－－－
	...
	const ATTACHED_HU_ZIMOFAN = 66; 			//自摸
	...
	self::HU_TYPE_PINGHU=>array(self::HU_TYPE_PINGHU, 2, '平胡')
	,self::HU_TYPE_QIDUI=>array(self::HU_TYPE_QIDUI, 4, '七对')
	);

	public static $attached_hu_arr = array(
	self::ATTACHED_HU_ZIMOFAN=>array(self::ATTACHED_HU_ZIMOFAN, 2, '自摸')
	...
	,self::ATTACHED_HU_MENQING=>array(self::ATTACHED_HU_MENQING, 2, '门清')	
	...
	public $m_yikouxiang = array(0,0,0,0);		// 一口香结构
	...
	public function InitDataSub()
	{
		...
		$this->m_yikouxiang = array(0,0,0,0);
		$this->m_cancle_time = 0;
	}

	public function _open_room_sub($params)
	{
        $this->m_rule = new RuleZhangJiaKouYiKouXiang();
		...
		$params['rule']['is_dianpao_type'] = !isset($params['rule']['is_dianpao_type']) ? 0 : $params['rule']['is_dianpao_type'];
		...
		$this->m_rule->is_dianpao_type = $params['rule']['is_dianpao_type'];
		...
		do {
			...
			foreach ($this->m_room_players as $key => $room_user)
			{
				if($room_user['uid'] == $params['uid'])
				{
					...
					if($this->m_yikouxiang[$key] == 0)
					{
						if(!empty($params['minglou']))
						{
							$this->m_yikouxiang[$key] = 1;
							//状态变化发消息
							$this->_send_act('s_yikouxiang', $key);
							$this->_set_record_game(ConstConfig::RECORD_YIKOUXIANG, $key);
						}
					}
					elseif($this->m_yikouxiang[$key] == 1)
					{
			...
			foreach ($this->m_room_players as $key => $room_user)
			{
				if($room_user['uid'] == $params['uid'])
				{
					...
					for ($i=0; $i<$this->m_rule->player_count; $i++)
					{
						...
						if( self::is_hu_give_up($temp_card, $this->m_nHuGiveUp[$last_chair]) || !$this->judge_hu($last_chair) || !empty($this->m_yikouxiang[$last_chair]))
						{
	...
	public function HandleOutCard($chair, $is_14 = false, $out_card = 0, $is_ting = 1)
	{
		...
		for ( $i=0; $i<$this->m_rule->player_count - 1; $i++)
		{
			...
			if(!empty($this->m_yikouxiang[$chair_next]))
			{
	...
	public function OnGetChairScene($chair, $is_more=false)
	{
		...
		$data['m_yikouxiang'] = $this->m_yikouxiang;      //一口香
	...
	public function ScoreOneHuCal($chair, &$lost_chair)
	{
		...
		if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)
		{
			$wWinScore += $PerWinScore + 2 * ConstConfig::SCORE_BASE;  //赢的分 加  自摸的分
		...
		else if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_CHI_PAO)
		{
			...
			if($this->m_rule->is_dianpao_type == 0) //点炮一家出
			{
				$this->m_wHuScore[$lost_chair] -= $wWinScore;
				$this->m_wHuScore[$chair] += $wWinScore;
			...
			elseif($this->m_rule->is_dianpao_type == 1)    //点炮大包
			{
				//点炮玩家要给赢家的分
				$wWinScore_tmp = ($this->m_rule->player_count - 1) * $wWinScore;
				
				$this->m_wHuScore[$lost_chair] -= $wWinScore_tmp;
				$this->m_wHuScore[$chair] += $wWinScore_tmp;

				$this->m_wSetLoseScore[$lost_chair] -= $wWinScore_tmp;
				$this->m_wSetScore[$chair] += $wWinScore_tmp;

				$this->m_HuCurt[$chair]->gain_chair[0] = 1;
				$this->m_HuCurt[$chair]->gain_chair[1]=$lost_chair;
			}
			elseif($this->m_rule->is_dianpao_type == 2)   //点炮三家出
			{
				for($i = 0; $i < $this->m_rule->player_count; $i++)
				{
					if($i == $chair)
