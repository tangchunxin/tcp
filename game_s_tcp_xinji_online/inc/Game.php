...
class Game
{
	...
	public function set_game_and_checkout($is_log=false)
	{
		...
		{
			$set_num = $this->m_rule->set_num / $this->m_rule->player_count;
		}
		else
		{
			$set_num = $this->m_rule->set_num ;
		}
		//扣费
		$result = Room::$get_conf;
		if(!empty($result['data']['room_type_circle']))
		{
        	$currency_tmp = BaseFunction::need_currency($result['data']['room_type'],$this->m_game_type,$set_num);
		}

		if(empty($result['data']['winner_currency']))
		{
			if($this->m_nSetCount == 1 && !($this->m_bWinBanker || $this->m_bNoCard ) )
			{
	...
	public function c_open_room($fd, $params)
	{
		...
		do {
			...
			$this->m_rule->is_circle = !isset($params['rule']['is_circle'])? 1 : $params['rule']['is_circle'];
			if($this->m_rule->is_circle)
			{
	...
	public function judge_hu($chair, $is_fanhun = false)
	{
		//胡牌型
		$is_yitiaolong = false;
		$hu_type = $this->judge_hu_type($chair,$is_yitiaolong);
	...
	//胡牌类型判断  没有混的情况
	public function judge_hu_type($chair, &$is_yitiaolong)
	{
		$jiang_arr = array();
		$qidui_arr = array();
		$gen_arr = array();
		...
		if($this->m_rule->is_feng && $i = ConstConfig::PAI_TYPE_FENG)
		{
			...
			if(0 < $this->m_sPlayer[$chair]->card[$i][0])
			{
				...
				else
				{
					...
					$gen_arr[] = intval($hu_list_val/512);
		...
		elseif($bQiDui && $this->m_rule->is_qidui_fan )				//判断七对，可能同时是32牌型
		{
		...
		return ConstConfig::HU_TYPE_PINGHU ;	//不是七对 就是 平胡
	}
	...
	public function judge_fan($chair)
	{
		...
		for($i=1; $i<$this->m_HuCurt[$chair]->count; $i++)
		{
			...
			{
				//最大胡牌类型
				$tmp_fan_sum = ConstConfig::$attached_hu_arr[$this->m_HuCurt[$chair]->method[$i]][1];
				if($max_fan_sum == 0)
				{
					$max_fan_sum = ConstConfig::$attached_hu_arr[$this->m_HuCurt[$chair]->method[$i]][1];
					$max_hu_desc = ConstConfig::$attached_hu_arr[$this->m_HuCurt[$chair]->method[$i]][2];
				}
				else
				{
					if($max_fan_sum <= $tmp_fan_sum)
					{
						$max_fan_sum = $tmp_fan_sum;
						$max_hu_desc = ConstConfig::$attached_hu_arr[$this->m_HuCurt[$chair]->method[$i]][2];
					}
				}
		...
		if($fan_sum <= $max_fan_sum)//辛集麻将  最大胡牌的类型
		{
			$fan_sum = $max_fan_sum;
			$tmp_hu_desc = $max_hu_desc;
		}
	...
	public function HandleSetOver()
	{
		...
		{
			$this->m_nChairBankerNext = $this->AntiClock($this->m_nChairBanker, 0);//荒庄 ,庄家不变
		}
		...
		{
			if($this->m_rule->is_circle && (1 == $this->m_bWinBanker || $this->m_bNoCard))
			{
				;
			}
			else
	...
	public function ScoreOneHuCal($chair, &$lost_chair)
	{
		...
		if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)
		{
			...
			for($i = 0; $i < $this->m_rule->player_count; $i++)
			{
				...
				$wWinScore = 0;
				$wWinScore += 2*$PerWinScore ;  //赢的分 加  庄家的分
	...
	public function CalcHuScore()
	{
		...
		for ($i = 0; $i<$this->m_rule->player_count; ++$i)
		{
			...
			{
				//退回刮风下雨,退税
				if ($j != $i && $this->m_wGangScore[$i][$j] > 0)
				{
	...
	public function WashCard()
	{
		...
		{
			$this->m_nCardBuf = ConstConfig::ALL_CARD_120;
			$this->m_nAllCardNum = 120;
		}
		else
		{
