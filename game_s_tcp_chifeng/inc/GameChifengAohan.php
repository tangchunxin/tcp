...
class GameChifengAohan
{
	//－－－－－－－－－－－－－游戏类型 －－－－－－－－－－－－－－－－－
    const GAME_TYPE = 401;                      //游戏类型

	//－－－－－－－－－－－－－胡牌类型 －－－－－－－－－－－－－－－－－
	const HU_TYPE_PINGHU = 21; 					// 平胡
	const HU_TYPE_JIAHU = 22; 					// 夹胡
    const HU_TYPE_PIAOHU = 23;                  // 飘胡
	...
	const ATTACHED_HU_GANGPAO   = 63; 			// 杠炮
    const ATTACHED_HU_SANBIMEN = 64;            //三闭门

	//－－－－－－－－－－－－－杠分 －－－－－－－－－－－－－－－－－－－
	const M_ZHIGANG_SCORE = 2;  				// 直杠  1分
	const M_ANGANG_SCORE = 4;   				// 暗杠  1分
	const M_WANGANG_SCORE = 2;  				// 弯杠  1分
	...
	,self::ATTACHED_HU_GANGPAO=>[self::ATTACHED_HU_GANGPAO, 2, '杠炮']
	,self::ATTACHED_HU_SANBIMEN=>[self::ATTACHED_HU_SANBIMEN, 4, '三闭门']
	);
	...
	//开新的房间或者加入房间,取房间状态（给http的server用）
	...
	public function c_open_room($fd, $params)
	{
		...
		do {
			...
			$this->m_rule = new RuleChifengaohan();
			if(empty($params['rule']['player_count']) || !in_array($params['rule']['player_count'], array(1, 2, 3, 4)))
			{
				$params['rule']['player_count'] = 4;
			}
            $params['rule']['set_num'] = !isset($params['rule']['set_num']) ? $params['rule']['is_circle'] : $params['rule']['set_num'];
            ...
            $params['rule']['pao_zi_score'] = !isset($params['rule']['pao_zi_score']) ? 0 : $params['rule']['pao_zi_score'];

			$this->m_rule->game_type = $params['rule']['game_type'];
			$this->m_rule->player_count = $params['rule']['player_count'];
			//$this->m_rule->set_num = $params['rule']['set_num'] * $this->m_rule->player_count;
			$this->m_rule->min_fan = $params['rule']['min_fan'];
			$this->m_rule->top_fan = $params['rule']['top_fan'];
			$this->m_rule->pao_zi_score = $params['rule']['pao_zi_score'];

            $this->m_rule->is_circle = 1;
            if(!empty($this->m_rule->is_circle))
            {
                $this->m_rule->set_num = ($params['rule']['set_num'] * $this->m_rule->player_count);		//局等于  人*圈
            }
            else
            {
	...
	public function judge_hu($chair)
	{
        ...
        //杠上开炮
        if ($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_CHI_PAO && $this->m_sGangPao->mark && $this->m_sGangPao->chair != $chair)
        {
        ...
        if ($this->m_rule->player_count==4)
        {
            ...
            {
                $this->m_HuCurt[$chair]->add_hu(self::ATTACHED_HU_SANBIMEN);
            }
        }
        return true;
	}
    ...
    public function judge_hu_type($chair)
    {
        ...
        if($i = ConstConfig::PAI_TYPE_FENG)
        {
            if(0 < $this->m_sPlayer[$chair]->card[$i][0])
            {
                ...
                {
                    if ($this->m_sPlayer[$chair]->card[$i][5] > 0 || $this->m_sPlayer[$chair]->card[$i][6] > 0 || $this->m_sPlayer[$chair]->card[$i][7] > 0)
                    {
                        $kezi_arr[]=1;
                    }
                    if($this->m_sPlayer[$chair]->card[$i][5] > 2 || $this->m_sPlayer[$chair]->card[$i][6] > 2 || $this->m_sPlayer[$chair]->card[$i][7] > 2 )
                    {
                        $musthupengpeng = true;
                    }
                    $hu_list_val = ConstConfig::$hu_data_feng[$key];
        ...
        for($i=0; $i<$this->m_sStandCard[$chair]->num; $i++)
        {
            ...
            else
            {
                ...
                if (in_array($this->m_sStandCard[$chair]->first_card[$i], array(53,54,55)))
                {
                    $musthupengpeng =true ;
                }
            }
        ...
        {
            if ($this->judge_32type($chair, array($this->m_HuCurt[$chair]->card, $this->m_HuCurt[$chair]->card + 1, $this->m_HuCurt[$chair]->card - 1)))
            {
                $bJiahu = true;
            }
		...
		else
		{
			...
			if($this->ScoreOneHuCal($chair, $tmp_lost_chair))
			{
				...
				if ($this->m_game_type == self::GAME_TYPE)
				{
					...
					$this->_set_record_game(ConstConfig::RECORD_ZIMO, $chair, $temp_card, $chair);$this->HandleSetOver();
	...
	//处理出牌
	public function HandleOutCard($chair, $is_14 = false, $out_card = 0,$is_ting = 0)
	{
        ...
        for ( $i=0; $i < $this->m_rule->player_count - 1; $i++)
        {
            ...
            if($this->_find_peng($chair_next)
                ...
                || ($tmp_distance == 1 && $this->_find_eat($chair_next,1) || $this->_find_eat($chair_next,2) || $this->_find_eat($chair_next,3))
            )
            {
	...
	public function ScoreOneHuCal($chair, &$lost_chair)  
	{
		...
		if($this->m_HuCurt[$chair]->state == ConstConfig::WIN_STATUS_ZI_MO)
		{
			...
			for($i = 0; $i < $this->m_rule->player_count; $i++)
			{
                ...
                //数据闭门判断
                if(!(in_array(self::ATTACHED_HU_SANBIMEN, $this->m_HuCurt[$chair]->method)))
                {
                    if($this->m_sStandCard[$i]->kai_men == false)
                    {
                        $banker_fan = $banker_fan * 2;
	...
	public function WriteScore()      
	{
		for($i = 0; $i < $this->m_rule->player_count; $i++)
		{
			...
			if (!empty($this->m_rule->pao_zi_score))
            {
                if($this->m_paozi_score[$i]>0)
                {
                    $this->m_hu_desc[$i] .= '铺分+'.$this->m_paozi_score[$i].' ';
                }
                else
                {
                    $this->m_hu_desc[$i] .= '铺分'.$this->m_paozi_score[$i].' ';
	...
	private function _set_record_game($act, $param_1 = 0, $param_2 = 0, $param_3 = 0, $param_4 = 0)
	{
		$param_1_tmp = 0;
		$param_3_tmp = 0;
		if(in_array($act, [ConstConfig::RECORD_CHI, ConstConfig::RECORD_PENG, ConstConfig::RECORD_ZHIGANG, ConstConfig::RECORD_ANGANG, ConstConfig::RECORD_ZHUANGANG, ConstConfig::RECORD_HU, ConstConfig::RECORD_HU_QIANGGANG, ConstConfig::RECORD_ZIMO, ConstConfig::RECORD_DISCARD, ConstConfig::RECORD_DRAW, ConstConfig::RECORD_DEALER]))
		{
			if(is_array($param_1))
			{
	...
	/*private function _set_game_and_checkout($is_log=false)
	{
		...
		$result = Room::$get_conf;
        if(!empty($result['data']['room_type_circle']))
        {
    ...
    private function _set_game_and_checkout($is_log=false)
    {
        ...
        if($tmp_game_info && $this->m_nSetCount != 255)	//非投票解散的牌局
        {
            var_dump(BaseFunction::web_curl(array('mod'=>'Business', 'act'=>'set_game_log', 'platform'=>'gfplay', 'rid'=>$this->m_room_id,'uid'=>$this->m_room_owner, 'uid_arr'=>implode(',', $uid_arr)
            , 'game_info'=>json_encode($tmp_game_info, JSON_UNESCAPED_UNICODE),'type'=>1, 'is_room_over'=>$is_room_over
            , 'game_type'=>$this->m_game_type, 'play_time'=>$itime - $this->m_start_time)));
        }
    ...
    public function judge_32type($chair,$delCard_arr)
    {
        ...
        {
            $this->_list_delete($chair,$delCard);
        }
        ...
        foreach ($delCard_arr as $delCard)
        {
            $this->_list_insert($chair,$delCard);
        }
        return $result;
    }
    //判断是否符合32牌型
    ...
    //根据开房的规则给每个人的下炮子
    private function _handle_pao_zi()
    {
        if(!empty($this->m_rule->pao_zi_score))
        {
            ...
            for ($i = 0; $i<$this->m_rule->player_count; ++$i)
            {
                $this->m_own_paozi[$i]->num = $this->m_rule->pao_zi_score;
                $tmp_paozi_arr[$i] = $this->m_rule->pao_zi_score;
            }
